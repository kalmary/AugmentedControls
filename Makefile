...
all: $(DIRS) $(FILES) .gitignore .gitattributes tree

# Tworzenie katalogów
$(DIRS):
	mkdir -p $@

# Tworzenie plików – upewniamy się, że katalog istnieje
$(FILES): | $(DIRS)
	touch $@

# Tworzenie .gitignore z zawartością Python
.gitignore: | $(DIRS)
	@echo "Creating .gitignore with Python template..."
...
clean:
	# Usuwamy pliki najpierw (including .gitignore)
	rm -f $(FILES) .gitignore .gitattributes
	# Usuwamy katalogi
	rm -rf $(DIRS)
